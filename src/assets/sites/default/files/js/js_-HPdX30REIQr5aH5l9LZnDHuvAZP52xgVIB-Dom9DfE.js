    ...
    function addClassToBody(cls) {
      $('body').addClass(cls);
		...
		if (is_mt && mobileAndTabletcheck()) {
			addClassToBody(options.mt_class);
		}
		if (is_ie) {
			var ie = detectIE();
			if (ie) addClassToBody('IE IE'+ie);
		}
		if (is_ios) {
			if (detectiOS()) addClassToBody('iOS');
		}
		if (is_load) {
      if (window.attachEvent)  {
        window.attachEvent('onload', addClassToBody('loaded'));
      } else if (window.addEventListener) {
        window.addEventListener('load', addClassToBody('loaded'), false);
      } else {
        document.addEventListener('load', addClassToBody('loaded'), false);
      }
		};
...
;
/*
  Select customization v 1.5 (02.04.20)
  Author: Pavel Tkach
*/
(function($){
        ...
        if ( $w && !$w.hasClass('wrap-sb') ) {
          ...
          $(this).children('option').each(function(i) {
            ...
            if (!$se.attr('multiple') || ( $se.attr('multiple') && i != 0 )) {
              $li = $('<li '+(this.disabled?'class="disabled"':'')+' data-value="'+$(this).val()+'">'+$(this).text()+'</li>').appendTo($u);
...
/**
 * @file
 * Simple JavaScript aimigrate file.
 */
  ...
  Drupal.behaviors.aimigrate = {
    attach: function (context, settings) {
      $(document).ready(function() {
        ...
        if(jQuery(location).attr('href').indexOf('bible') > 0 ){
          let bcv = getQueryParams(document.location.search);
          if(bcv){
            let span =  $('span.bcv');
            span.each(function( index ) {
              let value = $( this ).text(), clp = $( this ).parent().attr('class');
              $( this ).attr('id','bcv'+value);
              if (!clp) $( this ).parent().addClass('pof');
              if (Number(bcv) == Number(value)) {
               return $( this ).scrollView(-140);
              }
            });
          }
        }
...
(function ($, Drupal, cookies) {
  ...
  Drupal.behaviors.usccb_readings = {
    attach: function (context, settings) {
      ...
      // main-menu
       ...
       if ($main_menu.length) {
        ...
        if ( $('.book-page .contentarea').length>0 ) {
          var fl1 = false, fl2 = false;
          $('.book-page .contentarea a[name] > span.bcv').each(function() {
            var $p = $(this).parent();
            $p.after($p.html());
            $p.html('');
          });
          $('.book-page .contentarea p').each(function(i1) {
            //console.log('<p class="'+this.className+'"> ',i1);
            //console.log('before ->  ',$(this).html());
            var verse = [],str = '',sn, sv;
            verse = $(this).html().split('<span class="bcv">');
            for (var i = 0 ; i < verse.length; i++) {
              if ( i > 0 ){
                fl1 = true;
                sn = verse[i].slice(0,verse[i].indexOf('</span>')+7);
                sv = verse[i].slice(verse[i].indexOf('</span>')+7);
                sv = '<div class="verse"><span class="bcv">'+sn+'<span class="txt">'+sv+'</span></div>';
              } else sv = verse[i];
              str += sv;
            }
            $(this).html(str);
            if (fl1 && verse.length == 1) {
              $(this).addClass('wv');
              fl2 = true;
            }
            if (verse.length > 1) {
          ...
          $('.book-page .contentarea b + a.fnref').each(function(i) {
              var  $a = $(this);
              $(this).prev().append($a);
          });
        ...
        if ( $('.b-verse').length>0 ) {
          $('.b-verse h2 + h5').each(function(i) {
              var  $a = $(this).addClass('address'),$p = $(this).prev().addClass('name');
              $p.add($a).wrapAll('<div class="content-header"></div');
          });
          $('body').addClass('loaded');
        };
        ...
        // scroll to the ancor
        var ancor = new RegExp("^#");
        $('.book-page a').each(function(){
        ...
        $('.b-books-bottom-panel,.b-books-side-panel',context).each(function() {
          ...
          ovm = ( cookies.get(mes) == null);
          if ( !ovm ) $ov.addClass('hide');
          $ov.on('click','.icon-cancel',function() {
            var exp = 7; //days
            cookies.set(mes, true, { expires: exp });
            $ov.addClass('hide');
          });;
        });
        eventColorC();
